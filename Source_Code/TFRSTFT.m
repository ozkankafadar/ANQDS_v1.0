function [TFr] = TFRSTFT(X,T,N,FSW,Trace)
...
if (nargin == 1),
 T=1:xrow; FSW = TFTB_Window(hlength); Trace=0;
 elseif (nargin == 2) | (nargin == 3),
 FSW = tftb_window(hlength); Trace = 0;
elseif (nargin == 4),
 Trace = 0;
end;
...
[hrow,hcol]=size(FSW); Lh=(hrow-1)/2; 
...
for icol=1:tcol,
 ...
 TFr(indices,icol)=X(ti+tau,1).*conj(FSW(Lh+1+tau));
end;
TFr=fft(TFr); 
if Trace, fprintf('\n'); end;
if (nargout==0),
